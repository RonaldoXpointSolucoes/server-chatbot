...
router.post('/conversations/:conversationId/sync-history', requireTenant, async (req, res) => {
    try {
        ...
        console.warn(`[Sync-History] API Baileys >= v7.0: Histórico só é carregado on-startup via evento. Pulando extração de RAM para JID: ${jid}. O histórico persistido já está no Supabase.`);
