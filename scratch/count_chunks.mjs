...
async function run() {
    const docId = 'a10b8136-519d-42dc-ba35-8d7e5f65f4d9';
    const { data: chunks, error } = await supabaseClient.from('knowledge_chunks').select('id, chunk_index').eq('document_id', docId);
    console.log({ error, count: chunks ? chunks.length : 0, chunks });
}
run();
