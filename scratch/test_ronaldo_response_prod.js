...
const envDev = fs.readFileSync('.env', 'utf8');

const supabaseUrl = envProd.match(/VITE_SUPABASE_URL=(.*)/)[1].trim().replace(/^"(.*)"$/, '$1');
const serviceKey = envDev.match(/SUPABASE_SERVICE_ROLE_KEY=(.*)/)[1].trim().replace(/^"(.*)"$/, '$1');
const geminiKey = envProd.match(/VITE_GEMINI_API_KEY=(.*)/)[1].trim().replace(/^"(.*)"$/, '$1');
