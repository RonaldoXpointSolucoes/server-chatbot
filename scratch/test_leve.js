...
process.env.VITE_GEMINI_API_KEY = activeKey;
...
async function run() {
  ...
  // Vamos usar o bot "Luna Menu" pois foi ele quem respondeu "LUNA MENU"
  const botData = bots.find(bot => bot.name === 'Luna Menu') || bots[0];
  ...
  console.log("=== Chamando generateResponse (Primeira Vez) ===");
  try {
  ...
  console.log("\n=== Chamando generateResponse (Segunda Vez - Deve bater no Cache) ===");
  try {
    const responseText = await AutomationWorker.generateResponse(params);
    console.log("=== Resposta Gerada pela IA ===");
    console.log(responseText);
