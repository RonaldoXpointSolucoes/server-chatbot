        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#202c33] rounded-3xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
             ...
             <form onSubmit={handleCreateCompany} className="space-y-4">
                ...
                <div>
                   <label className="text-xs font-semibold text-[#54656f] uppercase tracking-wider">Grupo Econômico (Opcional)</label>
                   <select value={newCompany.economic_group_id} onChange={e => setNewCompany({...newCompany, economic_group_id: e.target.value})} className="w-full mt-1 bg-[#f0f2f5] dark:bg-[#111b21] dark:text-[#aebac1] border border-transparent rounded-xl px-4 py-3 outline-none focus:border-[#00a884]">
                      <option value="">Nenhum Grupo</option>
                      {economicGroups.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                   </select>
                </div>
                <div>
          ...
          <div className="bg-white dark:bg-[#202c33] rounded-3xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
             ...
             <form onSubmit={handleUpdateCompany} className="space-y-4">
                ...
                <div>
                   ...
                   <select required value={showEditCompany.status || 'active'} onChange={e => setShowEditCompany({...showEditCompany, status: e.target.value})} className="w-full mt-1 bg-[#f0f2f5] dark:bg-[#111b21] dark:text-white border border-transparent rounded-xl px-4 py-3 outline-none focus:border-[#00a884]">
                ...
                <div>
                   <label className="text-xs font-semibold text-[#54656f] uppercase tracking-wider">Grupo Econômico (Opcional)</label>
                   <select value={showEditCompany.economic_group_id || ''} onChange={e => setShowEditCompany({...showEditCompany, economic_group_id: e.target.value})} className="w-full mt-1 bg-[#f0f2f5] dark:bg-[#111b21] dark:text-white border border-transparent rounded-xl px-4 py-3 outline-none focus:border-[#00a884]">
                      <option value="">Nenhum Grupo</option>
                      {economicGroups.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                   </select>
                </div>
