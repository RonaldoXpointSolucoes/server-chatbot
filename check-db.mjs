...
async function run() {
    const {data, error} = await supabaseClient.from('contacts').select('id, name, phone, evolution_remote_jid');
    if (error) console.error(error);
    else fs.writeFileSync('contacts.json', JSON.stringify(data, null, 2));
}
run();
